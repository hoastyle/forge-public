...
- `knowledge get` is the read-only publication status surface for one knowledge document; responses now include `knowledge_kind`, the publication state, and `last_receipt_ref` so callers can trace the latest durable document state.
- `explain insight` is the read-only evidence trace surface for one insight receipt; excluded documents include `knowledge_kind` and `excluded_reason`, and `reference` knowledge is retrieval-only rather than synthesis evidence.
